...
export default {
  ...
  computed: {
    ...mapGetters(['company', 'departmentName', 'name', 'avatar'])
  },
  data() {
    return {
      /**
       * @type {Data,ProvidentFund,SocialInsurance}
       */
      homeData: {},
      messageList: []
    }
  ...
  async created() {
    // this.getHomeData()
    // this.getMessageList()
    this.homeData = await getHomeDataAPI()
    this.messageList = await getMessageListAPI()
  },
  mounted() {
    // 获取展示的数据 设置给图表
    // 监听homeData的变化
